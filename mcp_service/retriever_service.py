...
class LegalRetrieverService:
    ...
    def _search_retriever(retriever, query: str, limit: int):
        ...
        if callable(search):
            return search(
                query,
                top_k=limit,
                candidate_k=max(limit * settings.retrieval_candidate_multiplier, limit),
                rerank=settings.retrieval_rerank,
                hybrid=False,
                rewrite=settings.retrieval_query_rewrite,
            )
    ...
    def search(self, tool_name: str, query: str, limit: int = 3) -> str:
        ...
        return "\n\n".join(
            f"[{metadata_of(item).get('law_name', '未知法律')} {metadata_of(item).get('article', '')}"
            f" | {metadata_of(item).get('source', '')} p.{metadata_of(item).get('page', '')}"
            f" | score={score_of(item):.3f} | rerank_score={getattr(item, 'rerank_score', None)!s}]\n{content_of(item)}"
            for item in results
        )
