"""Phase 5 Qwen3.5-4B LoRA training on structured Agent trajectories."""
...
def _trajectory_to_text(item: dict, tokenizer) -> str:
    tool = item.get("tool", {})
    target = {
        "domain": item.get("domain"), "intent": item.get("intent"), "plan": item.get("plan", []),
        "tool": {"name": tool.get("name"), "arguments": tool.get("arguments", {})},
        "evidence": item.get("citations", []), "answer": item.get("answer", ""),
    }
    messages = [
...
def _load_data() -> list[dict]:
    ...
    if settings.hard_example_path.exists(): paths.append(settings.hard_example_path)
    rows, seen = [], set()
    for path in paths:
        ...
        with Path(path).open("r", encoding="utf-8") as f:
            for line in f:
                ...
                row = json.loads(line); key = row.get("question", "")
                if key and key not in seen: seen.add(key); rows.append(row)
    if not rows: raise FileNotFoundError("没有 trajectory 数据，请先运行 distill/trajectory.py")
    return rows
...
def train() -> None:
    ...
    dataset = Dataset.from_list([{"text": _trajectory_to_text(row, tokenizer)} for row in _load_data()])
    ...
    print(f"Qwen3.5-4B LoRA 已保存：{settings.qwen35_lora_output_dir}")
