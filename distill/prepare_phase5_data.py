"""Prepare Phase 5 decision and answer SFT datasets."""
...
def _load(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            ...
            row=json.loads(line); q=str(row.get("question","")).strip()
            if q and q not in seen:
                seen.add(q); rows.append(row)
    if not rows: raise ValueError("trajectory 数据为空")
    return rows
...
def _decision(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"messages":[
        {"role":"system","content":"你是法律 Agent。根据用户问题选择法域、意图和合适的法律检索工具。只输出合法 JSON，不要解释。"},
...
def _answer(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"messages":[
        {"role":"system","content":"你是法律 Agent。只能依据提供的检索证据回答；证据不足时明确说明。不要编造法条或事实。"},
...
def prepare() -> tuple[int,int]:
    ...
    decisions=[_decision(r) for r in rows if (r.get("tool") or {}).get("name")]
    answers=[_answer(r) for r in rows if str(r.get("answer","")).strip()]
