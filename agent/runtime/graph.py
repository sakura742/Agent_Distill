...
from .nodes import generation, intent_analysis, route_after_intent, task_planning, retrieval, retry_plan, route_after_tool, route_after_verification, tool_decision, tool_execution, verification
from .state import AgentState
from .tool_executor import DirectToolExecutor

def build_legal_agent_graph(*, tool_service=None, tool_executor=None, answer_generator: Callable | None = None, decision_generator: Callable | None = None):
    ...
    graph.add_conditional_edges("intent_analysis", route_after_intent, {"legal": "task_planning", "non_legal": "generation"})
    graph.add_edge("task_planning", "tool_decision")
    graph.add_edge("tool_decision", "tool_execution")
    graph.add_conditional_edges("tool_execution", route_after_tool, {"retrieval": "retrieval", "retry_or_end": "retry_plan"})
    graph.add_edge("retrieval", "generation")
    graph.add_edge("generation", "verification")
    graph.add_conditional_edges("verification", route_after_verification, {"end": END, "retry": "retry_plan"})
