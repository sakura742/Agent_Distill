...
def tool_execution(state: AgentState, executor=None) -> AgentState:
    if not state.get("tool_name"):
        return {"tool_result": "", "error": None, "trace": _trace(state, "tool_execution", tool=None, status="skipped")}
...
def verification(state: AgentState) -> AgentState:
    ...
    ok = bool(answer.strip()) if non_legal else bool(answer.strip()) and bool(citations)
    reason = "non_legal_answer_present" if non_legal and ok else ("answer_and_citations_present" if ok else "missing_answer_or_citations")
    v = {"passed": ok, "citation_count": len(citations), "reason": reason}
