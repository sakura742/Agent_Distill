...
class LegalRetriever:
    def __init__(self, domain: str, *, embedding_model_name: str | None = None, chroma_db_dir: str | None = None, validate_index: bool = True):
        ...
        try:
            self.vectorstore = Chroma(collection_name=self.collection, embedding_function=self.embeddings, persist_directory=self.chroma_db_dir)
    ...
    def _merge_query_variants(*variant_results: list[RetrievedChunk]) -> list[RetrievedChunk]:
        ...
        for results in variant_results:
            for item in results:
                key = str(item.metadata.get("chunk_id") or (item.metadata.get("law_name", ""), item.metadata.get("article", ""), item.metadata.get("source", ""), item.content))
                existing = merged.get(key)
                if existing is None or item.score > existing.score:
                    merged[key] = item
        return sorted(merged.values(), key=lambda item: item.score, reverse=True)

    def search(self, query: str, *, top_k: int = 5, candidate_k: int | None = None, law_name: str | None = None, article: str | None = None, rerank: bool = False, hybrid: bool = False, rewrite: bool = False) -> list[RetrievedChunk]:
    ...
    def _rerank(query: str, results: list[RetrievedChunk]) -> list[RetrievedChunk]:
        ...
        if not model_name:
            raise KnowledgeBaseError("已请求 rerank，但 AGENT_DISTILL_RERANKER_MODEL 未配置。请配置可用的 CrossEncoder 模型后再运行 rerank 实验。")
        try:
            from sentence_transformers import CrossEncoder
            model = CrossEncoder(model_name)
            scores = model.predict([(query, item.content) for item in results])
            reranked = [RetrievedChunk(item.content, item.metadata, item.score, item.distance, float(raw_score)) for raw_score, item in zip(scores, results)]
            return sorted(reranked, key=lambda item: float(item.rerank_score or 0.0), reverse=True)
        except Exception as exc:
            raise KnowledgeBaseError(f"CrossEncoder reranker 加载或推理失败: {model_name!r}") from exc


def get_retriever(domain: str, *, embedding_model_name: str | None = None, chroma_db_dir: str | None = None) -> LegalRetriever:
